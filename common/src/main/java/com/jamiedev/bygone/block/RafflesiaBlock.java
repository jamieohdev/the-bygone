    ...
    public void animateTick(BlockState state, Level world, BlockPos pos, @NotNull RandomSource random) {
        ...
        for(int l = 0; l < 14; ++l) {
            ...
            if (!blockState.isCollisionShapeFullBlock(world, mutable)) {
                world.addParticle((ParticleOptions) JamiesModParticleTypes.RAFFLESIA_SPORES, (double)mutable.getX() + random.nextDouble(), (double)mutable.getY() + random.nextDouble(), (double)mutable.getZ() + random.nextDouble(), 0.0, 0.0, 0.0);
            }
        }
