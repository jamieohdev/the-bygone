...
public class PointedAmberBlock   extends Block implements LandingBlock2, Waterloggable {
    ...
    public MapCodec<PointedAmberBlock> getCodec() {
        return CODEC;
    }

    public PointedAmberBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    protected void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (isPointingDown(state) && !this.canPlaceAt(state, world, pos)) {
            ...
            if (isHeldByPointedAmber(state, world, pos)) {
                ...
                if (optional.isPresent()) {
                    ...
                    if (!(dripChance >= f)) {
                        ...
                        if (blockPos != null) {
                            if (((PointedAmberBlock.DrippingFluid)optional.get()).state.isOf(Blocks.MUD) && fluid == Fluids.WATER) {
                                BlockState blockState = Blocks.CLAY.getDefaultState();
                                world.setBlockState(((PointedAmberBlock.DrippingFluid)optional.get()).pos, blockState);
                                Block.pushEntitiesUpBeforeBlockChange(((DrippingFluid)optional.get()).state, blockState, world, ((PointedAmberBlock.DrippingFluid)optional.get()).pos);
                                world.emitGameEvent(GameEvent.BLOCK_CHANGE, ((PointedAmberBlock.DrippingFluid)optional.get()).pos, GameEvent.Emitter.of(blockState));
    ...
    public void onDestroyedOnLanding(World world, BlockPos pos, RisingBlockEntity fallingBlockEntity) {
        if (!fallingBlockEntity.isSilent()) {
            world.syncWorldEvent(9045, pos, 0);
        }
    ...
    private static void spawnRisingBlock(BlockState state, ServerWorld world, BlockPos pos) {
        BlockPos.Mutable mutable = pos.mutableCopy();

        for(BlockState blockState = state; isPointingUp(blockState); blockState = world.getBlockState(mutable)) {
            RisingBlockEntity fallingBlockEntity = RisingBlockEntity.spawnFromBlock(world, mutable, blockState);
            if (isTip(blockState, true)) {
                int i = Math.max(1 + pos.getY() - mutable.getY(), 6);
                float f = (float) i;
                fallingBlockEntity.setHurtEntities(f, 40);
    ...
    private static Thickness getThickness(WorldView world, BlockPos pos, Direction direction, boolean tryMerge) {
        ...
        if (isPointedAmberFacingDirection(blockState, direction2)) {
            ...
            if (thickness != Thickness.TIP && thickness != Thickness.TIP_MERGE) {
                ...
                return !isPointedAmberFacingDirection(blockState2, direction) ? Thickness.BASE : Thickness.MIDDLE;
            } else {
    ...
    private static boolean canPlaceAtWithDirection(WorldView world, BlockPos pos, Direction direction) {
        ...
        BlockState blockState = world.getBlockState(blockPos);
        return blockState.isSideSolidFullSquare(world, blockPos, direction) || isPointedAmberFacingDirection(blockState, direction);
    }
    ...
    private static boolean isHeldByPointedAmber(BlockState state, WorldView world, BlockPos pos) {
    ...
    public static BlockPos getDripPos(World world, BlockPos pos) {
        ...
        return (BlockPos)searchInDirection(world, pos, Direction.UP.getDirection(), biPredicate, PointedAmberBlock::canDrip, 11).orElse((BlockPos) null);
    }
    ...
    private static Optional<PointedAmberBlock.DrippingFluid> getFluid(World world, BlockPos pos, BlockState state) {
