...
public class ShelfMyceliumBlock  extends SpreadableBlock implements Fertilizable
{
    GrassBlock ref;
    public static final MapCodec<ShelfMyceliumBlock> CODEC = createCodec(ShelfMyceliumBlock::new);

    public MapCodec<ShelfMyceliumBlock> getCodec() {
        return CODEC;
    }
    ...
    public ShelfMyceliumBlock(Settings settings) {
        super(settings);
    }
            ...
            if (world.getLightLevel(pos.up()) >= 0) {
                ...
                for(int i = 0; i < 4; ++i) {
                    ...
                    if (world.getBlockState(blockPos).isOf(JamiesModBlocks.LIMBOSTONE) && canSpread(blockState, world, blockPos)) {
                        world.setBlockState(blockPos, (BlockState)blockState.with(SNOWY, world.getBlockState(blockPos.up()).isOf(JamiesModBlocks.SHELF_MYCELIUM)));
                    }
                }
            }
