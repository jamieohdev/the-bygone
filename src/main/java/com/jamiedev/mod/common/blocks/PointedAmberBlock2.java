...
public class PointedAmberBlock2  extends Block implements LandingBlock, Waterloggable {
    public static final MapCodec<PointedAmberBlock2> CODEC = createCodec(PointedAmberBlock2::new);
    ...
    public MapCodec<PointedAmberBlock2> getCodec() {
        return CODEC;
    }

    public PointedAmberBlock2(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public static void dripTick(BlockState state, ServerWorld world, BlockPos pos, float dripChance) {
        if (!(dripChance > 0.17578125F)) {
            if (isHeldByPointedDripstone(state, world, pos)) {
                Optional<PointedAmberBlock2.DrippingFluid> optional = getFluid(world, pos, state);
                if (optional.isPresent()) {
                    Fluid fluid = ((PointedAmberBlock2.DrippingFluid)optional.get()).fluid;
                    float f;
                    ...
                    if (!(dripChance >= f)) {
                        ...
                        if (blockPos != null) {
                            if (((PointedAmberBlock2.DrippingFluid)optional.get()).sourceState.isOf(Blocks.MUD) && fluid == Fluids.WATER) {
                                BlockState blockState = Blocks.CLAY.getDefaultState();
                                world.setBlockState(((PointedAmberBlock2.DrippingFluid)optional.get()).pos, blockState);
                                Block.pushEntitiesUpBeforeBlockChange(((PointedAmberBlock2.DrippingFluid)optional.get()).sourceState, blockState, world, ((PointedAmberBlock2.DrippingFluid)optional.get()).pos);
                                world.emitGameEvent(GameEvent.BLOCK_CHANGE, ((PointedAmberBlock2.DrippingFluid)optional.get()).pos, GameEvent.Emitter.of(blockState));
    ...
    private static void place(WorldAccess world, BlockPos pos, Direction direction, Thickness thickness) {
        BlockState blockState = (BlockState)((BlockState)((BlockState) JamiesModBlocks.POINTED_AMBER.getDefaultState().with(VERTICAL_DIRECTION, direction)).with(THICKNESS, thickness)).with(WATERLOGGED, world.getFluidState(pos).getFluid() == Fluids.WATER);
    ...
    private static BlockPos getTipPos(BlockState state, WorldAccess world, BlockPos pos, int range, boolean allowMerged) {
        if (isTip(state, allowMerged)) {
            ...
            BiPredicate<BlockPos, BlockState> biPredicate = (posx, statex) -> {
                return statex.isOf(JamiesModBlocks.POINTED_AMBER) && statex.get(VERTICAL_DIRECTION) == direction;
            };
    ...
    private static Optional<BlockPos> getSupportingPos(World world, BlockPos pos, BlockState state, int range) {
        ...
        return searchInDirection(world, pos, direction.getOpposite().getDirection(), biPredicate, (statex) -> {
            return !statex.isOf(JamiesModBlocks.POINTED_AMBER);
        }, range);
    }
    ...
    private static boolean isHeldByPointedDripstone(BlockState state, WorldView world, BlockPos pos) {
        return isPointingDown(state) && !world.getBlockState(pos.up()).isOf(JamiesModBlocks.POINTED_AMBER);
    }
    ...
    private static boolean isPointedDripstoneFacingDirection(BlockState state, Direction direction) {
        return state.isOf(JamiesModBlocks.POINTED_AMBER) && state.get(VERTICAL_DIRECTION) == direction;
    }

    @Nullable
    private static BlockPos getCauldronPos(World world, BlockPos pos, Fluid fluid) {
        Predicate<BlockState> predicate = (state) -> {
            return canDripThrough(world, pos, state);
        };
    ...
    public static BlockPos getDripPos(World world, BlockPos pos) {
        ...
        return (BlockPos)searchInDirection(world, pos, Direction.UP.getDirection(), biPredicate, PointedAmberBlock2::canDrip, 11).orElse((BlockPos) null);
    }
    ...
    public static Fluid getDripFluid(ServerWorld world, BlockPos pos) {
        return (Fluid)getFluid(world, pos, world.getBlockState(pos)).map((fluid) -> {
            return fluid.fluid;
        }).filter(PointedAmberBlock2::isFluidLiquid).orElse(Fluids.EMPTY);
    }

    private static Optional<PointedAmberBlock2.DrippingFluid> getFluid(World world, BlockPos pos, BlockState state) {
        return !isPointingDown(state) ? Optional.empty() : getSupportingPos(world, pos, state, 11).map((posx) -> {
            ...
            return new PointedAmberBlock2.DrippingFluid(blockPos, (Fluid)fluid, blockState);
        });
    }
    ...
    private static boolean canGrow(BlockState dripstoneBlockState, BlockState waterState) {
        return dripstoneBlockState.isOf(JamiesModBlocks.COBBLED_AMBER);
    }
    ...
    static record DrippingFluid(BlockPos pos, Fluid fluid, BlockState sourceState) {
        ...
        static  Fluid flui11;
        static BlockState sourceState1;
